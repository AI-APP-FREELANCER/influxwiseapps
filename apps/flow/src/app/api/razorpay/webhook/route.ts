...
export async function POST(req: Request) {
  ...
  await db.paymentEvent.upsert({
    where: { id: payment.id },
    update: {},
    create: { id: payment.id, type: event.event, app: "flow", payload: event as object },
  });

  if (event.event === "payment.captured") {
    const notes = payment.notes ?? {};
    if (notes.app === "flow" && notes.userId) {
      const amountPaise = payment.amount;

      const wallet = await db.flowWallet.upsert({
        where: { userId: notes.userId },
        update: { balancePaise: { increment: amountPaise } },
        create: { userId: notes.userId, balancePaise: amountPaise },
      });

      await db.flowTopup.create({
        data: {
          walletId: wallet.id,
          amountPaise,
